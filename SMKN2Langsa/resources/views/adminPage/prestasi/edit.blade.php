...
@section('judul', 'Ubah Data Prestasi')

@section('container')

    <div class="section-body p-4 bg-white">
        <form action="/prestasis/{{ $prestasi->id }}" method="post" enctype="multipart/form-data" class="ewrapper">
            @method('patch')
            @csrf
            <div class="form-group">
                <label>Judul Prestasi</label>
                <input type="text" class="form-control @error('judul') is-invalid @enderror" name="judul" value="{{ $prestasi->judul }}" placeholder="Masukan judul prestasi">
                @error('judul')
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('lokasi') is-invalid @enderror" name="lokasi" value="{{ $prestasi->lokasi }}" placeholder="Masukan Lokasi">
                @error('lokasi')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="form-group">
                <label>Tanggal</label>
                <input type="date" class="form-control @error('tanggal') is-invalid @enderror" name="tanggal" value="{{ $prestasi->tanggal }}">
                @error('tanggal')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="form-group">
                <label for="desc">Teks Prestasi</label>
                    <div class="toolbar" style="display: none;">
                      ...
                      <div class="form-group">
                        <textarea name="teks" class="editable form-control @error('teks') is-invalid @enderror" value="{{ $prestasi->teks }}" style="height: 500px;" placeholder="Masukkan teks prestasi">{{ $prestasi->teks }}</textarea>
                        @error('teks')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                      </div>
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('teks_singkat') is-invalid @enderror" name="teks_singkat" value="{{ $prestasi->teks_singkat }}" placeholder="Masukan Teks Prestasi Singkat">
                        @error('teks_singkat')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
