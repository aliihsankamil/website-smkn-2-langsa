...
@section('container')
    ...
    <div class="section-body p-4 bg-white">
        <form action="{{ route('tambah_data_berita') }}" method="post" enctype="multipart/form-data" class="ewrapper">
            @csrf
            <div class="form-group">
                <label>Judul Berita</label>
                <input type="text" class="form-control @error('judul') is-invalid @enderror" name="judul" value="{{ old('judul') }}" placeholder="Masukan judul berita">
                @error('judul')
            ...
            <div class="form-group">
                <label for="desc">Teks berita</label>
                    <div class="toolbar" style="display: none;">
                      ...
                      <div class="form-group">
                        <textarea name="teks" class="editable form-control @error('teks') is-invalid @enderror" value="{{ old('teks') }}" style="height: 500px;" placeholder="Masukkan teks berita">{{ old('teks') }}</textarea>
                        @error('teks')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                      </div>
                    <div class="form-group">
                        <label>Teks berita singkat | contoh:berita ini merupakan berita yang</label>
                        <input type="text" class="form-control @error('teks_singkat') is-invalid @enderror" name="teks_singkat" value="{{ old('teks_singkat') }}" placeholder="Masukan Teks Berita Singkat">
                        @error('teks_singkat')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <a href="{{ route('data_berita') }}" class="btn btn-danger">Kembali</a>
        </form>
    </div>
