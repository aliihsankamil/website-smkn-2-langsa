...
@section('judul', 'Edit Data Sejarah')

@section('container')

    <div class="section-body bg-white p-4 p-4 bg-white">
        <form method="POST" action="/histories/{{ $history->id }}" class="ewrapper">
            @method('patch')
            @csrf
            <div class="form-group">
                ...
                <textarea class="editable form-control  @error('sejarah') is-invalid @enderror" name="sejarah" value="{{ $history->teks }}" style="height: 500px;" placeholder="Masukan Teks">{{ $history->teks }}</textarea>
                @error('sejarah')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <button type="submit" class="btn btn-primary">Edit Teks</button>
            <a href="{{ url('/histories') }}" class="btn btn-danger">Kembali</a>
        </form>
    </div>
