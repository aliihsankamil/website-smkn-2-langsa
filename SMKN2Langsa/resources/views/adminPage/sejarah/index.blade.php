...
@section('title', 'Data Sejarah Sekolah')
@section('judul', 'Data Sejarah Sekolah')
...
@section('container')
    ...
    <div class="section-body bg-white p-4 p-4 bg-white mb-4">
        ...
        @endif
        <form method="POST" action="{{ route('tambah_history_proses') }}" class="ewrapper">
            @csrf
            <div class="form-group">
                ...
                <textarea class="editable form-control mt-2 @error('sejarah') is-invalid @enderror" name="sejarah" value="{{ old('sejarah') }}" style="height: 500px;" placeholder="Masukan Teks" ></textarea>
                @error('sejarah')
        ...
        <hr/>
        <h4>Data Sejarah Sekolah</h4>
        @foreach ($history as $hs)
            <p>
                {!! $hs->teks !!}
                <a href="/histories/{{ $hs->id }}/edit" class="btn btn-primary">
                    Edit
                </a>
                <button type="submit" data-id="{{ $hs->id }}" class="btn btn-danger swal-confirm">
                    <form action="{{ route('delete_history', $hs->id) }}" id="delete{{ $hs->id }}" method="POST">
                        @csrf
                        @method('delete')
                    </form>
                    Hapus
