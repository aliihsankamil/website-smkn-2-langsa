...
@section('title', 'Visi Sekolah')
@section('judul', 'Visi Sekolah')
...
@section('container')
    ...
    <div class="section-body bg-white p-4">
        ...
        @endif
        {{-- <a href="{{route('tambah_visi')}}" class="btn btn-primary mb-4">Tambah Data Misi</a> --}}

        <form method="POST" action="{{ route('tambah_visi_proses') }}" class="ewrapper">
            @csrf
            <div class="form-group">
                ...
                <textarea class="editable form-control mt-2 @error('visi') is-invalid @enderror" name="visi" value="{{ old('visi') }}" style="height: 500px;" placeholder="Masukan Visi" ></textarea>
                @error('visi')
        ...
        <hr/>
        <h4>Visi Sekolah</h4>
        @foreach ($vision as $vs)
            <p class="mb-0">{!! $vs->teks !!}</p>
            <a href="{{ route('edit_visi',$vs->id) }}" class="btn btn-primary">
                Edit
            </a>
            <button type="submit" data-id="{{ $vs->id }}" class="btn btn-danger swal-confirm">
                <form action="{{ route('delete_visi', $vs->id) }}" id="delete{{ $vs->id }}" method="POST">
                    @csrf
                    @method('delete')
                </form>
                Hapus
