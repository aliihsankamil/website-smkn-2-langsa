...
@section('container')
    ...
    <div class="section-body p-4 bg-white">
        <form action="{{ route('tambah_data_jurusan') }}" method="post" enctype="multipart/form-data" class="ewrapper">
            @csrf
            <div class="form-group">
                <label>Nama Jurusan</label>
                <input type="text" class="form-control @error('nama') is-invalid @enderror" name="nama" value="{{ old('nama') }}" placeholder="Masukan nama jurusan">
                @error('nama')
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('kaprodi') is-invalid @enderror" name="kaprodi" value="{{ old('kaprodi') }}" placeholder="Masukan Nama kaprodi">
                @error('kaprodi')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="form-group">
                <label>Foto Depan Jurusan</label>
                <input type="file" class="form-control @error('kaprodi') is-invalid @enderror" name="foto">
                @error('kaprodi')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="form-group">
                <label for="desc">Teks Paragraf</label>
                    ...
                    <textarea name="desc" class="editable form-control @error('desc') is-invalid @enderror" value="{{ old('desc') }}" style="height: 500px;" placeholder="Input per paragraf"></textarea>
                    @error('desc')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            </div>
