...
@section('title', 'Edit Misi Sekolah')
@section('judul', 'Edit Misi Sekolah')

@section('container')

    <div class="section-body bg-white p-4">
        <form method="POST" action="{{ route('edit_misi_proses',$data_misi->id) }}" class="ewrapper">
            @csrf
            @method('patch')
            ...
            <textarea class="editable form-control mt-2 @error('misi') is-invalid @enderror" name="misi"
            @if (old('misi'))
                value="{{ old('misi') }}"
            @else
                value="{{ $data_misi->teks }}"
            @endif
            style="height: 500px;" placeholder="Masukan Misi" >{{ $data_misi->teks }}</textarea>
            ...
            <a href="{{route('data_misi')}}" class="btn btn-danger my-3">Kembali</a>
        </form>
    </div>
